...
# Makefile melhorado para SIABES
...
# Prefix/project name (prefira COMPOSE_PROJECT_NAME)
PREFIX ?= ${COMPOSE_PROJECT_NAME}
COMPOSE_PROJECT_NAME ?= $(PREFIX)
export COMPOSE_PROJECT_NAME

# Perfil (dev, local, prod...) -> usado em 'up' e outras operações
# Ex.: make up PROFILE=dev
PROFILE ?= ${COMPOSE_PROJECT_NAME}

# serviços (lê DOCKER_SERVICE_* do .env)
SERVICE_NAMES := $(shell grep -E '^[[:space:]]*DOCKER_SERVICE_' .env 2>/dev/null | sed 's/[[:space:]]*//g' | cut -d '=' -f2 | tr '\n' ' ')

# Fallbacks para nomes de serviços que o Makefile utiliza diretamente
DOCKER_SERVICE_PHP_FPM ?= ${DOCKER_SERVICE_PHP_FPM:-DOCKER_SERVICE_PHP_FPM}
DOCKER_SERVICE_NGINX   ?= ${DOCKER_SERVICE_NGINX:-siabes-nginx}
DOCKER_SERVICE_PGSQL   ?= ${DOCKER_SERVICE_PGSQL:-pgsql}
